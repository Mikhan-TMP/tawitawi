...
class Auth extends CI_Controller
{
  ...
  private function _login()
  {
          ...
          if ($user['is_verified'] == 1 && empty($user['verification_token'])) {
              if (password_verify($password, $user['password'])) {
                  $data = [
                      ...
                      'device' => $device, 
                      'is_loggedIn' => 1,
                     'login_msg' => "Welcome back, " . $user['username'] . "!",
                  ];

                  $this->session->set_userdata($data);
                //   $this->handleMissingTimeouts();
      ...
      if ($user) {
        ...
        if($this->session->userdata('device') == 'desktop') {
            $this->session->set_flashdata('login_error', 'Username not found. Please try again.');
            redirect('auth');
        }else {
            echo $log_inc_uname;
            $this->session->set_flashdata('login_error', 'Username not found. Please try again.');
  ...
  private function handleLoginError($user, $log_code)
  {
      switch ($user['role_id']) {
          case 1:
              $this->session->set_flashdata('login_error', 'Wrong password. Please try again.');
              redirect('auth');
              break;
          case 2:
              if ($this->session->userdata('device') == 'desktop') {
  ...
  {
      if ($this->session->userdata('device') == 'desktop') {
          $this->session->set_flashdata('login_error', 'Verification is needed. Please check your email.');
          redirect('auth');
      } else {
          echo $log_code;
          $this->session->set_flashdata('login_error', 'Verification is needed. Please check your email.');
  ...
  public function logout()
  {
    ...
    // change the is_loggedin to 0
    $this->session->set_userdata('is_loggedIn', 0);
    $this->session->set_flashdata('logout_message', 'Logged out Successful!');
    redirect('auth');
  }
      ...
      if ($this->form_validation->run() == false) {
          ...
          if ($user) {
              ...
              if (!$uppercase || !$lowercase || !$number || !$specialChars || strlen($npassword) < 8) {
                  $this->session->set_flashdata('fpass_fail', 'Please make more strong password (Combine numbers, lowercase, and uppercase letters and special characters)', TRUE);
                  redirect('auth/forgotpassword');
              }

              if ($this->input->post('password') != $this->input->post('cpassword')) {
                  $this->session->set_flashdata('fpass_fail', 'Password not match!');
              } else {
                  ...
                  if ($rows > 0) {
                    ...
                    if ($this->email->send()) {
                        $this->session->set_flashdata('fpass_scs', 'Password Changed!, Please check your email to confirm');
                    } else {
                        $this->session->set_flashdata('fpass_fail', 'Error sending verification email. Please try again later.');
                    }
                      // $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
                      //     Successfully Update Password!</div>');
                  } else {
                      $this->session->set_flashdata('fpass_fail', 'Failed to edit Password!');
                  }
              }
              redirect('auth');
          } else {
              $this->session->set_flashdata('fpass_fail', 'Username not found!');
              redirect('auth/forgotpassword');
          }
      }
  }
  public function verification($verification_token) {
    ...
    if ($user) {
        ...
        $this->session->set_flashdata('fpass_scs', 'Successfully Update Password!');
    } else {
        $this->session->set_flashdata('fpass_fail', ' Invalid verification token.');
    }
    ...
    //     );
      ...
      if ($this->form_validation->run() == FALSE) {
        ...
        //   $this->load->view('auth/account');
          ...
          if ($this->email->send()) {
              $this->session->set_flashdata('success', 'Account has been created! Please check your email to verify your account.');
          } else {
              $this->session->set_flashdata('error', 'Error sending verification email. Please try again later.');
          }
...
public function getAllBookings() {
    ...
    // Debug the data to see if it's properly structured
    echo '<pre>';
    print_r($bookings);
    echo '</pre>';
    // After confirming data structure, use JSON encoding

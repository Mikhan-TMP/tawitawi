...
public function area()
  {    
    $d['title'] = 'Library Area Informaiton';
  ...
  public function e_area($d_id)
  {
    ...
    if ($this->form_validation->run() == false) {
      ...
      $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
      Successfully edited a area!</div>');
      redirect('master/area');
    }
  }
  ...
  public function d_area($d_id)
  {
    ...
    $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
        Successfully deleted a area!</div>');
    redirect('master/area');
  }
  /*************************************   Room  *********************************************/
...
/************************************************************************ */
  ...
  {
    $d['title'] = 'Librian Access Control';
    $d['account'] = $this->Admin_model->getAdmin($this->session->userdata['username']);

    $d['data'] = $this->db->get('user_submenu')->result_array();
    $d['access'] = $this->db->get('user_access')->result_array();
    $d['menu'] = $this->db->get('user_menu')->result_array();
    $this->load->view('templates/table_header', $d);
    $this->load->view('templates/sidebar');
    $this->load->view('templates/topbar');
    $this->load->view('master/groups/index', $d); // Department Page
    $this->load->view('templates/table_footer');
  }
